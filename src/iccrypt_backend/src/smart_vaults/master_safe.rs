...
impl MasterSafe {
    ...
    pub fn get_or_create_user_safe(&mut self, owner: &UserID) -> &UserSafe {
        self.user_safes().entry(*owner).or_insert(UserSafe::new(owner)) // Get an existing user_safe for the owner or create a new one
    }
    ...
    pub fn add_secret(&mut self, owner: &UserID, secret: &Secret) {
        let user_safe = self.user_safes().entry(*owner).or_insert(UserSafe::new(owner)); // Get an existing user_safe for the owner or create a new one
...
mod tests {
    ...
    async fn utest_get_or_create_user_safe() {
        ...
        assert_eq!(user_safe_1a.owner() , owner.id(), "user_safe_1a should have owner: {},  but has {}", owner.id(), user_safe_1a.owner());
        assert_eq!(user_safe_1a.secrets().len(), 0, "user_safe_1a should have 0 secrets but has {}", user_safe_1a.secrets().len());
        ...
        assert_eq!(user_safe_1b.owner() , owner.id(), "user_safe_1b should have owner: {},  but has {}", owner.id(), user_safe_1b.owner());
        assert_eq!(user_safe_1b.secrets().len(), 0, "user_safe_1b should have 0 secrets but has {}", user_safe_1b.secrets().len());
        assert_eq!(master_safe.user_safes().len() , 1, "master_safe should have 1 user_safe but has {}", master_safe.user_safes().len());
    ...
    async fn utest_secrets() {
        ...
        let mut secret = Secret::new(owner.id(), &SecretCategory::Password, &String::from("my-super-secret")).await;
        master_safe.add_secret(owner.id(), &secret);
        assert_eq!(master_safe.get_or_create_user_safe(owner.id()).secrets().len(), 1, "user_safe should have 1 secrets but has {}", master_safe.get_or_create_user_safe(owner.id()).secrets().len());
        let secret_name = master_safe.get_or_create_user_safe(owner.id()).secrets().get(secret.id()).unwrap().name();
        assert_eq!(secret_name, &String::from("my-super-secret"), "secret should have name my-super-secret but has {}", secret_name);

        secret.set_name(&String::from("my-super-secret-new"));
        master_safe.update_secret(owner.id(), &secret);
        let secret_name = master_safe.get_or_create_user_safe(owner.id()).secrets().get(secret.id()).unwrap().name();
        assert_eq!(secret_name, "my-super-secret-new", "secret should have name my-super-secret-new but has {}", secret_name);

        master_safe.remove_secret(owner.id(), &secret.id());
        assert_eq!(master_safe.get_or_create_user_safe(owner.id()).secrets().len(), 0, "user_safe should have 0 secrets but has {}",master_safe.get_or_create_user_safe(owner.id()).secrets().len());
    }
}